...
=head1 NAME
    ...
    rprepnorm.pm - version 1.3

=head1 OVERVIEW of rprepnorm

    This is the wrapper for prepnorm. It is intended to operate in two modes: 

=over 4

=item main()

    Called when the user wants to run prepnorm and alter the input values 
    specific to prepnorm. Meant specifically for command line interface.
    Not written yet ..

=item run()

    Called when the user just wants to run prepnorm and not change any of the 
    input values.
...
=item $GLOBE::prepOutput
    ...
    Zero for minimum process output from prepnorm and greater than zero 
    otherwise.
...
$rprepnorm::logname = "prepnorm.log";
$rprepnorm::inpname = "prep_s.inp";
...
=item string check()
    ...
    the input file for PREPNORM. Returns a string of errors if
...
sub clean{
    ...
    &File::remove($rprepnorm::logname);

    $rprepnorm::inpname=~s/_[s,v,c]/_s/;
    &File::remove($rprepnorm::inpname);
    $rprepnorm::inpname=~s/_[s,v,c]/_v/;
    &File::remove($rprepnorm::inpname);
    $rprepnorm::inpname=~s/_[s,v,c]/_c/;
    &File::remove($rprepnorm::inpname);
...
=item int run(void)
    ...
    Runs prepnorm and redirects output from the screen to the logfile.
...
sub run{
	...
	if(&File::openWrite($rprepnorm::logname)){
	    open(LOGFILE,">>$rprepnorm::logname") || 
                 die "Could not open $rprepnorm::logname: $!";
	    $rprepnorm::date=&Stdio::getDate();
	    print LOGFILE "******************** $rprepnorm::date\n";
	    close(LOGFILE) || die "Could not close $rprepnorm::logname: $!";
	    (print STDOUT "running prepnorm\n")unless($xpdf::exist);

            my $runcmd=File::Spec->canonpath("$GLOBE::execpath/prepnorm");
            system("$runcmd < $rprepnorm::inpname >> $rprepnorm::logname")==0
                   or die "Could not run prepnorm: $!";

	    &convert();
	    $val=&File::runOk('prepnorm');
	}
    } else {
	print STDERR 
        "$rprepnorm::inpname could not be written so prepnorm will not run\n";
	$val=0;
    }
    return $val;
}
...
sub writeInput{
    ...
    if(&File::openWrite($rprepnorm::inpname)){
	&OS::openFile(">$rprepnorm::inpname") || 
             die "Could not open $rprepnorm::inpname: $!";
	...
	print GLOBE::FILE "\n";
	close(GLOBE::FILE) || die "Could not close $rprepnorm::inpname: $!";
    }
...
=item void convert(void)
    ...
    Converts the bin file produced by prepnorm (.int) into an ascii file (.ain)
