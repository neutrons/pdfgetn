...
=head1 NAME
    ...
    rprepsepd.pm - version 1.3

=head1 OVERVIEW of rprepsepd

    This is the wrapper for prepsepd. It is intended to operate in two modes: 

=over 4

=item main()

    Called when the user wants to run prepsepd and alter the input values 
    specific to prepsepd. Meant specifically for command line interface.
    Not written yet ..

=item run()

    Called when the user just wants to run prepsepd and not change any of the 
    input values.
...
=item $GLOBE::backNumber

    Run number of the background measurement.
...
=item $GLOBE::neutFunc
    ...
    Can take a value of {0,1,2}. If zero no correction will be made; if 1 
    the flat neutron background is subtracted so $GLOBE::neutMinLambda and 
    $GLOBE::neutMaxLambda must be specified; if 2 there will also be a 
    sawtooth correction made so $GLOBE::neutStartLambda and 
    $GLOBE::neutEndLambda must also be specified.
...
=item $GLOBE::neutEndLambda

    A variable that needs to be specified if $GLOBE::neutFunc is two.

=item $GLOBE::firstMonitor

    The bank number of the first monitor, usually 5.
...
=item $GLOBE::neutMaxLambda

    A variable that needs to be specified if $GLOBE::neutFunc is 
    greater than one.

=item $GLOBE::neutMinLambda
...
=item $GLOBE::prepOutput
    ...
    Zero for minimum process output from prepsepd and greater than zero 
    otherwise.

=item $GLOBE::neutFrac

    This is the fraction of decayed neutrons.
...
=item $GLOBE::secondMonitor

    The number of the second monitor, if greater than one there is no 
    second monitor.

=item $GLOBE::neutStartLambda
...
$rprepsepd::logname = "prepsepd.log";
$rprepsepd::inpname = "prep_s.inp";
...
=item string check()
    ...
    the input file for PREPSEPD. Returns a string of errors if
...
sub check{
    ...
    unless(&body::cInt($GLOBE::firstMonitor)){
	$return.="$start firstMonitor as an integer $edit.";
    }
    unless(&body::cInt($GLOBE::secondMonitor)){
	$return.="$start secondMonitor as an integer $edit.";
    }
...
#		    }
    ...
    unless(&body::cInt($GLOBE::neutFunc)){
	$return.="$start neutFunc as an integer $edit.";
    }
    unless(&body::cFloat($GLOBE::neutFrac)){
	$return.="$start neutFrac as a number $edit.";
    }
    unless(&body::cFloat($GLOBE::neutMinLambda)){
	$return.="$start minLambda as a number $edit.";
    }
    unless(&body::cFloat($GLOBE::neutMaxLambda)){
	$return.="$start maxLambda as a number $edit.";
    }
    unless(&body::cFloat($GLOBE::neutStartLambda)){
	$return.="$start startLambda as a number $edit.";
    }
    unless(&body::cFloat($GLOBE::neutEndLambda)){
	$return.="$start endLambda as a number $edit.";
    }
    ...
    unless(&body::cInt($GLOBE::numCal)){
	$return.="$start numCal as an integer $edit.";
    }else{
	for( my $int=0 ; $int<$GLOBE::numCal ; $int++ ){
	    unless(&body::cFloat($GLOBE::bankAngle[$int])){
		unless($return=~/bankAngle for prep/){
		    $return.="$start bankAngle for prep as a number $edit.";
		}
	    }
	    unless(&body::cFloat($GLOBE::prepDifc[$int])){
		unless($return=~/difC for prep/){
		    $return.="$start difC for prep as a number $edit.";
		}
	    }
	    unless(&body::cFloat($GLOBE::prepDifa[$int])){
		unless($return=~/difA for prep/){
		    $return.="$start difA for prep as a number $edit.";
		}
	    }
	    unless(&body::cFloat($GLOBE::prepTzero[$int])){
		unless($return=~/Tzero for prep/){
		    $return.="$start Tzero for prep as a nubmer $edit.";
		}
	    }
	}
    }
...
sub clean{
    ...
    &File::remove($rprepsepd::logname);

    $rprepsepd::inpname=~s/_[s,c,v]/_c/;
    &File::remove($rprepsepd::inpname);
    $rprepsepd::inpname=~s/_[s,c,v]/_v/;
    &File::remove($rprepsepd::inpname);
    $rprepsepd::inpname=~s/_[s,c,v]/_s/;
    &File::remove($rprepsepd::inpname);
...
=item void noOut(void)

    This removes the output from prepsepd.
...
=item int run(void)
    ...
    Runs prepsepd and redirects output from the screen to the logfile.
...
sub run{
	...
	if(&File::openWrite($rprepsepd::logname)){
	    open(LOGFILE,">>$rprepsepd::logname") || 
                 die "Could not open $rprepsepd::logname: $!";
	    $rprepsepd::date=&Stdio::getDate();
	    print LOGFILE "******************** $rprepsepd::date\n";
	    close(LOGFILE) || 
                 die "Could not close $rprepsepd::logname: $!";
	    (print STDOUT "running prepsepd\n")unless($xpdf::exist);

	    my $runcmd=File::Spec->canonpath("$GLOBE::execpath/prepsepd");
	    system ("$runcmd < $rprepsepd::inpname >> $rprepsepd::logname") 
                   == 0 or die "Could not run prepsepd: $!";

	    &convert();
	    $val=&File::runOk('prepsepd');
	}
    } else {
	print STDERR 
        "$rprepsepd::inpname could not be written so prepsepd will not run\n";
	$val=0;
    }
    return $val;
}
...
sub writeInput{
    ...
    if(&File::openWrite($rprepsepd::inpname)){
	&OS::openFile(">$rprepsepd::inpname") || 
             die "Could not open $rprepsepd::inpname: $!";
	...
	print GLOBE::FILE "$GLOBE::firstMonitor,$GLOBE::secondMonitor,";
	...
	print GLOBE::FILE "$GLOBE::neutFunc,$GLOBE::neutFrac,";
	print GLOBE::FILE "$GLOBE::neutMinLambda,";
	print GLOBE::FILE "$GLOBE::neutMaxLambda,";
	print GLOBE::FILE "$GLOBE::neutStartLambda,";
	print GLOBE::FILE "$GLOBE::neutEndLambda\n";

	print GLOBE::FILE "$GLOBE::numCal\n";
	for( $int=0 ; $int<$GLOBE::numCal ; $int ++ ){
		print GLOBE::FILE "$GLOBE::bankAngle[$int],";
		print GLOBE::FILE "$GLOBE::prepDifc[$int],";
		print GLOBE::FILE "$GLOBE::prepDifa[$int],";
		print GLOBE::FILE "$GLOBE::prepTzero[$int]\n";
	}
	...
	print GLOBE::FILE "\n";
	close(GLOBE::FILE) || die "Could not close $rprepsepd::inpname: $!";
...
sub writehist{
    ...
    ## line 03a: monitor banks and hitogram number
    print Hist::HFILE "firstMonitor=$GLOBE::firstMonitor     ";
    print Hist::HFILE "secondMonitor=$GLOBE::secondMonitor\n";

    ## line 03b: parameters for delayed neutron correction
    print Hist::HFILE "neutFunc=$GLOBE::neutFunc     ";
    print Hist::HFILE "neutFrac=$GLOBE::neutFrac\n";
    print Hist::HFILE "neutMinLambda=$GLOBE::neutMinLambda     ";
    print Hist::HFILE "neutMaxLambda=$GLOBE::neutMaxLambda\n";
    print Hist::HFILE "neutStartLambda=$GLOBE::neutStartLambda     ";
    print Hist::HFILE "neutEndLambda=$GLOBE::neutEndLambda\n";

    ## line 03c: parameters for delayed neutron correction
    print Hist::HFILE "numCal=$GLOBE::numCal\n";
    print Hist::HFILE "Angle        DifC       DifA        Tzero\n";
    for ($int=0; $int<$GLOBE::numCal; $int++) {
      printf Hist::HFILE "%8.2f  ", $GLOBE::bankAngle[$int];
      printf Hist::HFILE "%10.2f  ",$GLOBE::prepDifc[$int];
      printf Hist::HFILE "%10.5f  ",$GLOBE::prepDifa[$int];
      printf Hist::HFILE "%10.5f\n",$GLOBE::prepTzero[$int];
    }
...
sub readhist{
    ...
    ## line 03a: monitor banks and hitogram number
    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
    ($GLOBE::firstMonitor)=(split /\s+/,$line)[0];
    $GLOBE::firstMonitor=~s/\QfirstMonitor=\E//;
    ($GLOBE::secondMonitor)=(split /\s+/,$line)[1];
    $GLOBE::secondMonitor=~s/\QsecondMonitor=\E//;

    ## line 03b-d: parameters for delayed neutron correction
    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
    ($GLOBE::neutFunc)=(split /\s+/,$line)[0];
    $GLOBE::neutFunc=~s/\QneutFunc=\E//;
    ($GLOBE::neutFrac)=(split /\s+/,$line)[1];
    $GLOBE::neutFrac=~s/\QneutFrac=\E//;

    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
    ($GLOBE::neutMinLambda)=(split /\s+/,$line)[0];
    $GLOBE::neutMinLambda=~s/\QneutMinLambda=\E//;
    ($GLOBE::neutMaxLambda)=(split /\s+/,$line)[1];
    $GLOBE::neutMaxLambda=~s/\QneutMaxLambda=\E//;

    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
    ($GLOBE::neutStartLambda)=(split /\s+/,$line)[0];
    $GLOBE::neutStartLambda=~s/\QneutStartLambda=\E//;
    ($GLOBE::neutEndLambda)=(split /\s+/,$line)[1];
    $GLOBE::neutEndLambda=~s/\QneutEndLambda=\E//;

    ## line 03c: parameters for delayed neutron correction
    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
    ($GLOBE::numCal)=(split /\s+/,$line)[0];
    $GLOBE::numCal=~s/\QnumCal=\E//;
    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
    for ($int=0; $int<$GLOBE::numCal; $int++) {
      chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
      $GLOBE::bankAngle[$int]=(split /\s+/,$line)[0];
      $GLOBE::prepDifc[$int]=(split /\s+/,$line)[1];
      $GLOBE::prepDifa[$int]=(split /\s+/,$line)[2];
      $GLOBE::prepTzero[$int]=(split /\s+/,$line)[3];
    }
...
=item void convert(void)
    ...
    Converts the bin file produced by prepsepd (.int) into an ascii file (.ain)
...
sub convert{
    ...
    system ("$runcmd $filename $outfile") == 0 or 
           die "Could not run int2asc: $!";
}
...
sub getTitle{
    ...
    while (<RUN>) {
      if (/^$tag/) {$dummy=<RUN>; chomp($dummy); last;}
    }
    close (RUN);
    return $dummy;
}

1;    # return true if loaded into another program
