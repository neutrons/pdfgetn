...
C
C $Id: gladprep1.f,v 1.1 2006/03/12 09:59:25 tproffen Exp $
C
C $Log: gladprep1.f,v $
...
C basic input information is on standard input
      ...
      call uGladReadInputs()
      call uGladGetNumberOfBanks(data_file(1),num_of_det_groups)
      call uGladInitialize()

C Read some basic info from the data file.

      user_name = 'User unknown'
      run_title = 'Created with prepnorm ..'
...
C propagation.
     ...
     1           ' ...'
            ...
            ind_min=-1
            ind_max=-1
            call uGladReadBank1(data_file(1), isg, Q, data, data_err, 
     1           add_to_data(isg), mult_data(isg), num_chan, twotht, 
     1           dist, xf, source_to_sample, print_level,
     1           ind_min,ind_max)
            do k=2, num_data_sets
               write(6, *) '%%info: data set ', k
               call uGladReadBank1(data_file(k), isg, Q, data1, 
     1              data1_err, add_to_data(isg), mult_data(isg), 
     1              num_chan, twotht, dist, xf, source_to_sample,
...
C-Do the same for the background data, if it exists
     ...
     1             isg,' ...'
              ...
              call uGladReadBank1(bkg_file(1), isg, Q, bkg_data, 
     1             bkg_data_err, add_to_bkg(isg), mult_bkg(isg), 
     1             num_chan, twotht, dist, xf, source_to_sample,
     1             print_level,ind_min,ind_max)
              do k=2, num_bkg_sets
                 write(6, *) '%%info: background set', k
                 call uGladReadBank1(bkg_file(k), isg, Q, bkg_data1, 
...
C Treat here the vanadium rod. Case 1: A background will be subtracted.
           ...
           if (lvan) then
              call uQtoLambda1(Q, lambda, twotht, num_chan)
              if (lbgd) then
     ...
     1                   print_level, vanthreshold)                    
                    if (print_level .ge. 1) then
                       call uKuplotOut1(25, out_file, ".vsmo", 
     1                      Q, data, data_err, num_chan, isg,
     1                      twotht)
                    endif
                 endif
                 if (bkg_smth_input .eq. 1) then
C                    call uQtoLambda1(Q, lambda, twotht, num_chan)
                    if (print_level .ge. 1) then 
                       call uKuplotOut1(25, out_file, ".vraw", 
     1                      Q, data, data_err, num_chan, isg,
     1                      twotht)
...
C                    call uQtoLambda1(Q, lambda, twotht, num_chan)
     ...
     1                      Q, data, data_err, num_chan, isg,
     1                      twotht)
                       call uKuplotOut1(25, out_file, ".csmo", 
     1                      Q, data, data_err, num_chan, isg,
     1                      twotht)
                    endif
                 endif
...
C close output file
      ...
      write(6, *) 'successful termination of prepnorm '
