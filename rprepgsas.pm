...
=head1 NAME
    ...
    rprepgsas.pm - version 1.0

=head1 OVERVIEW of rprepgsas

    This is the wrapper for prepgsas. It is intended to operate in two modes: 

=over 4

=item run()

    Called when the user just wants to run prepgsas and not change any of the 
    input values.
...
=item $GLOBE::prepOutput
    ...
    Zero for minimum process output from prepgsas and greater than zero 
    otherwise.
...
$rprepgsas::logname = "prepgsas.log";
$rprepgsas::inpname = "prep_s.inp";
...
=item string check()
    ...
    the input file for PREPGSAS. Returns a string of errors if
...
sub check{
    ...
    unless($GLOBE::instParamFile){
    $return.="$start an instrument parameter file.";
    } else {
    unless (File::checkExist($GLOBE::instParamFile)) {
        $return.="\nInstrument parameter file $GLOBE::instParamFile does not exist.";
    }
    }
...
sub clean{
    ...
    &File::remove($rprepgsas::logname);

    $rprepgsas::inpname=~s/_[s,v,c]/_s/;
    &File::remove($rprepgsas::inpname);
    $rprepgsas::inpname=~s/_[s,v,c]/_v/;
    &File::remove($rprepgsas::inpname);
    $rprepgsas::inpname=~s/_[s,v,c]/_c/;
    &File::remove($rprepgsas::inpname);
...
=item int run(void)
    ...
    Runs prepgsas and redirects output from the screen to the logfile.
...
sub run{
    ...
    if(&File::openWrite($rprepgsas::logname)){
        open(LOGFILE,">>$rprepgsas::logname") || 
                 die "Could not open $rprepgsas::logname: $!";
        $rprepgsas::date=&Stdio::getDate();
        print LOGFILE "******************** $rprepgsas::date\n";
        close(LOGFILE) || die "Could not close $rprepgsas::logname: $!";
        (print STDOUT "running prepgsas\n")unless($xpdf::exist);

            my $runcmd=File::Spec->canonpath("$GLOBE::execpath/prepgsas");
            system("$runcmd < $rprepgsas::inpname >> $rprepgsas::logname")==0
          or die "Could not run prepgsas: $!";

        &convert();
        $val=&File::runOk('prepgsas');
    }
    } else {
    print STDERR 
        "$rprepgsas::inpname could not be written so prepgsas will not run\n";
    $val=0;
    }
    return $val;
}
...
sub writeInput{
    ...
    if(&File::openWrite($rprepgsas::inpname)){
    &OS::openFile(">$rprepgsas::inpname") || 
             die "Could not open $rprepgsas::inpname: $!";
    ...
    print GLOBE::FILE "\n";
    close(GLOBE::FILE) || die "Could not close $rprepgsas::inpname: $!";
    }
...
sub writehist{
    ...
    ## line 02: Instrument parameter file
    print Hist::HFILE "instParamFile=$GLOBE::instParamFile\n";

    ## line 03: Additive corrections
    ...
    ## line 04: Multiplicatice corrections
...
sub readhist{
    ...
    unless ($GLOBE::fileExt) {$GLOBE::fileExt="gsa";}

    ## line 02: Instrument parameter file
    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
    ($GLOBE::instParamFile)=(split /\s+/,$line)[0];
    $GLOBE::instParamFile=~s/\QinstParamFile=\E//;
    ...
    ## line 04: Multiplicative corrections
    chomp($line=<Hist::HFILE>); $line=~s/^\s+//;
...
=item void convert(void)
    ...
    Converts the bin file produced by prepgsas (.int) into an ascii file (.ain)
...
=item void readhistHIPD()

    Reads the prep dependent part of the history file; OLD HIPD format !
...
sub readhistHIPD{
    ...
    ## Stuff that is not in the old file
    $GLOBE::numBanksMiss=0;
}
